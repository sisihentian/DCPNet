    ...
    def __init__(
        ...
        self.aggreg = nn.ModuleList(
            [
                nn.Sequential(
                    nn.Conv2d(
                        3 * total_dim, 3 * total_dim, scale, padding=get_same_padding(scale),
...
class EfficientViTBlock(nn.Module):
    def __init__(self, in_channels: int, heads_ratio: float = 1.0, dim=32, expand_ratio: float = 4,
...
class EfficientViTBackbone(nn.Module):
    def __init__(self, width_list: List[int], depth_list: List[int], in_channels=3, dim=32,
                 expand_ratio=4, norm="bn2d", act_func="hswish") -> None:
        super().__init__()
        ...
        self.channel = [i.size(1) for i in self.forward(torch.randn(1, 3, 384, 384))]  #

    @staticmethod
    def build_local_block(in_channels: int, out_channels: int, stride: int, expand_ratio: float,
                          norm: str, act_func: str, fewer_norm: bool = False) -> nn.Module:
        if expand_ratio == 1:
            block = DSConv(
                in_channels=in_channels,
...
def efficientvit_b0(weights='', **kwargs) -> EfficientViTBackbone:
    ...
    if weights:
        backbone.load_state_dict(
            update_weight(backbone.state_dict(), torch.load(weights)['state_dict']))
    return backbone
